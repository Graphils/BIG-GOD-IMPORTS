...
function ProductModal({ product, onClose, onSave }) {
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-md)', width: '100%', maxWidth: '720px', maxHeight: '90vh', overflow: 'auto', padding: '40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '28px', color: 'var(--navy)' }}>{product ? 'Edit Product' : 'Add New Product'}</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input name="tags" value={form.tags} onChange={handleChange} placeholder="e.g. imported, quality, sale" />
            </div>
            <div className="form-group">
          ...
          <div className="form-group">
            ...
            <div
              style={{ border: '2px dashed var(--border)', borderRadius: 'var(--radius-md)', padding: '32px', textAlign: 'center', cursor: 'pointer', transition: 'var(--transition)' }}
            ...
            >
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="1.5" style={{ margin: '0 auto 12px' }}><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
              <p style={{ color: 'var(--text-mid)', fontSize: '15px' }}>Click to upload or drag and drop</p>
              <p style={{ color: 'var(--text-light)', fontSize: '13px', marginTop: '4px' }}>PNG, JPG up to 5MB each. First image will be main.</p>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '12px', flexWrap: 'wrap' }}>
                {previews.map((src, i) => (
                  <div key={i} style={{ position: 'relative' }}>
                    <img src={src} alt="" style={{ width: '80px', height: '80px', objectFit: 'cover', borderRadius: 'var(--radius)', border: '2px solid var(--border)' }} />
              ...
              <div style={{ marginTop: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {product.images.map((img, i) => (
                    <img key={i} src={img.url} alt="" style={{ width: '72px', height: '72px', objectFit: 'cover', borderRadius: 'var(--radius)', border: '2px solid var(--border)' }} />
                  ))}
                </div>
              </div>
            )}
...
function StockModal({ product, onClose, onSave }) {
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-md)', padding: '40px', width: '100%', maxWidth: '400px' }}>
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '24px', color: 'var(--navy)', marginBottom: '8px' }}>Update Stock</h3>
        <p style={{ color: 'var(--text-light)', fontSize: '14px', marginBottom: '28px' }}>{product.name}</p>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <span className="form-error" style={{ color: 'var(--text-light)', fontSize: '12px' }}>Notify when stock falls to or below this number</span>
        </div>
...
export default function AdminProducts() {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Deactivate this product? It will no longer appear in the shop.')) return;
      ...
      <div style={{ background: 'var(--navy)', padding: '32px 0' }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            <h1 style={{ color: 'var(--white)', fontSize: 'clamp(22px,3vw,32px)', margin: '8px 0 4px' }}>Product Management</h1>
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px' }}>{total} products total</p>
          </div>
      ...
      <div className="container" style={{ padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: '1', minWidth: '200px', padding: '10px 16px', border: '1.5px solid var(--border)', borderRadius: 'var(--radius)', fontSize: '14px', background: 'var(--white)' }}
          />
          <select value={category} onChange={e => { setCategory(e.target.value); setPage(1); }}
            style={{ padding: '10px 16px', border: '1.5px solid var(--border)', borderRadius: 'var(--radius)', fontSize: '14px', background: 'var(--white)', color: 'var(--text-dark)' }}>
        ...
        <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
          {loading ? <div className="loading-page"><div className="spinner" /></div> : products.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-light)' }}>
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" style={{ margin: '0 auto 16px' }}><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
              ...
              <thead>
                <tr style={{ background: 'var(--navy)' }}>
                  {['Product', 'Category', 'Price', 'Stock', 'Status', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '14px 16px', textAlign: 'left', color: 'var(--gold)', fontSize: '12px', fontWeight: '600', letterSpacing: '1.5px', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={p._id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'var(--white)' : 'var(--cream)' }}>
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        {p.images?.[0]?.url
                          ? <img src={p.images[0].url} alt={p.name} style={{ width: '48px', height: '48px', objectFit: 'cover', borderRadius: 'var(--radius)', flexShrink: 0 }} />
                          : <div style={{ width: '48px', height: '48px', background: 'var(--cream)', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        ...
                        <div>
                          <p style={{ fontSize: '14px', fontWeight: '600', color: 'var(--navy)' }}>{p.name}</p>
                          <p style={{ fontSize: '12px', color: 'var(--text-light)' }}>{p.sku}</p>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '14px 16px', fontSize: '13px', color: 'var(--text-mid)' }}>{p.category}</td>
                    <td style={{ padding: '14px 16px' }}>
                      <p style={{ fontSize: '14px', fontWeight: '700', color: 'var(--navy)' }}>GHS {p.price.toFixed(2)}</p>
                      {p.comparePrice && <p style={{ fontSize: '12px', color: 'var(--text-light)', textDecoration: 'line-through' }}>GHS {p.comparePrice.toFixed(2)}</p>}
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ ...stockStatusStyle(p.stockStatus), padding: '4px 10px', borderRadius: '2px', fontSize: '11px', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase' }}>
