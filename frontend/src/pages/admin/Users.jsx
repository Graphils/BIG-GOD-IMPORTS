      ...
      <div className="container">
        <div className="admin-toolbar">
          <input
            ...
            style={{ padding: '10px 16px', border: '1.5px solid var(--border)', borderRadius: 'var(--radius)', fontSize: '14px', background: 'var(--bg-card)' }}
          />
        </div>
            ...
            <div className="admin-card" style={{ padding: 0, overflow: 'hidden' }}>
              <div className="admin-table-wrap order-table">
                <table>
                  ...
                  <tbody>
                    {filtered.map((user, i) => (
                      <tr key={user._id} style={{ borderBottom: '1px solid var(--border-color)', background: i % 2 === 0 ? 'var(--white)' : 'var(--cream)' }}>
                        <td style={{ padding: '14px 16px' }}>
                          <p style={{ fontWeight: '600', color: 'var(--text-heading)', fontSize: '14px', whiteSpace: 'nowrap' }}>{user.firstName || ''} {user.lastName || ''}</p>
              ...
              {filtered.map(user => (
                <div key={user._id} className="order-card">
                  <div className="order-card-header">
                    <div>
                      <p style={{ fontWeight: '700', color: 'var(--text-heading)', fontSize: '15px' }}>{user.firstName || ''} {user.lastName || ''}</p>
