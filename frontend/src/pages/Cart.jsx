...
export default function Cart() {
  ...
  if (!cartCount) return (
    <div>
      <div className="page-title"><div className="container"><h1>Shopping Cart</h1></div></div>
  ...
  return (
    <div>
      <div className="page-title"><div className="container"><div className="breadcrumb"><Link to="/">Home</Link><span>/</span><span>Cart</span></div><h1>Shopping Cart</h1></div></div>
            ...
            <div key={item.product._id} className="cart-item">
              <div className="cart-item-product">
                <Link to={`/product/${item.product._id}`}>
                  {item.product.images?.[0]?.url ? <img src={item.product.images[0].url} alt={item.product.name} /> : <div className="cart-img-placeholder"/>}
                </Link>
                <div className="cart-item-info">
                  <Link to={`/product/${item.product._id}`}><h4>{item.product.name}</h4></Link>
                  <button className="remove-btn" onClick={() => { removeFromCart(item.product._id); toast.success('Removed from cart.'); }}>Remove</button>
                </div>
              </div>
              ...
              <div className="cart-item-qty">
                ...
                <button onClick={() => updateQuantity(item.product._id, item.quantity + 1)} disabled={item.quantity >= item.product.stock}>+</button>
              </div>
